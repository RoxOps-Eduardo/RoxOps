...
function renderDashboard(d) {
  ...
  const metrics = [
    ...
    { icon: '💵', label: 'CAC', value: currency(cac), sub: `Ideal: ${currency(d.benchmarks.cac_maximo)} ${d.mes_anterior ? getTrend(cac, d.mes_anterior.cac, true) : ''}`, warn: cac > d.benchmarks.cac_maximo },
  ...
  if (d.agente) {
    const ag = d.agente;
    const agentMetrics = [
      { icon: '🤖', label: 'Agente Ativo', value: ag.nome, sub: 'Script/modelo em uso', cyan: true },
      { icon: '⚡', label: 'Taxa de Resposta', value: pct(ag.taxa_resposta), sub: `${ag.mensagens_respondidas} de ${ag.mensagens_enviadas} mensagens`, green: ag.taxa_resposta >= 0.75 },
      { icon: '⏱️', label: 'Tempo Médio de Resposta', value: ag.tempo_medio_resposta_seg + 's', sub: 'Tempo até primeira resposta do agente', cyan: true },
      { icon: '💬', label: 'Mensagens Enviadas', value: ag.mensagens_enviadas.toLocaleString('pt-BR'), sub: `Respondidas: ${ag.mensagens_respondidas}` },
    ];

    document.getElementById('agentGrid').innerHTML = agentMetrics.map(m => `
      <div class="metric-card animate-in">
        <div class="metric-icon">${m.icon}</div>
        <div class="metric-label">${m.label}</div>
  ...
  const benchRows = [
    ...
    { metric: 'CAC (Custo por Cliente)', atual: currency(cac), ideal: `Até ${currency(bm.cac_maximo)}`, ok: cac <= bm.cac_maximo },
  ...
  /* --- Channel Table --- */
  const channels = [
    { name: 'Google Ads', dot: 'google', inv: d.canais.google.investimento, leads: d.canais.google.leads },
    { name: 'Meta Ads', dot: 'meta', inv: d.canais.meta.investimento, leads: d.canais.meta.leads },
    { name: 'WhatsApp', dot: 'whatsapp', inv: d.canais.whatsapp.investimento, leads: d.canais.whatsapp.leads_organicos },
  ];

  document.getElementById('channelTableBody').innerHTML = channels.map(ch => {
      ...
      <tr>
        <td>
          <div class="channel-name">
            <span class="channel-dot channel-dot--${ch.dot}"></span>
            ${ch.name}
          </div>
        </td>
  ...
  const conversionMetrics = [
    { label: 'Lead → MQL', value: pct(taxaMql), sub: `${totalLeads} → ${d.funil.mql}`, ok: taxaMql >= bm.taxa_mql_minima },
    { label: 'MQL → SQL', value: pct(taxaSql), sub: `${d.funil.mql} → ${d.funil.sql}`, ok: taxaSql >= bm.taxa_sql_minima },
    { label: 'SQL → Oportunidade', value: pct(taxaOpp), sub: `${d.funil.sql} → ${d.funil.oportunidades}` },
    { label: 'Oportunidade → Venda', value: pct(taxaVenda), sub: `${d.funil.oportunidades} → ${vendas}`, ok: taxaVenda >= bm.taxa_conversao_minima },
